 ...
 * montrer — la couche STATIQUE courante, les batteries — voyage dans l'ACK de
 * leurs propres émissions, comme la sync keymap (prioritaire).
 * ⚠ SANS destinataire : les deux moitiés partagent le pipe 0 du dongle, et une
 * charge d'ACK part avec le PROCHAIN ACK, quel qu'en soit l'émetteur. Une trame
 * « pour la droite » était mangée par la gauche en USB (5 annonces/s) et la
 * droite, muette au repos, ne recevait jamais rien (banc 2026-09-14). La trame
 * porte donc les DEUX batteries et chaque moitié y lit celle de l'autre.
 * 5 octets : [type<<4][couche][batt G dV][batt D dV][chg G | chg D<<2 | ok<<4].
 * Testée host dans test/test_rf_packet.c. Spec : ecrans-memlcd-design.md §4. */
#define PKT_TYPE_DISPLAY  0xA
#define RF_DISPLAY_LEN    5

typedef struct {
    uint8_t couche;          /* couche statique (TO / base), jamais un MO tenu */
    uint8_t batt_dv[2];      /* [RF_HALF_LEFT], [RF_HALF_RIGHT] : dV, 0 = inconnue */
    uint8_t batt_chg[2];     /* 0 inconnu, 1 en charge probable, 2 pleine */
...
static inline uint16_t rf_encode_display(uint8_t *buf, const rf_display_t *d)
{
    ...
    buf[0] = (uint8_t)(PKT_TYPE_DISPLAY << 4);
    buf[1] = d->couche;
    buf[2] = d->batt_dv[RF_HALF_LEFT];
    buf[3] = d->batt_dv[RF_HALF_RIGHT];
    buf[4] = (uint8_t)((d->batt_chg[RF_HALF_LEFT] & 0x3) | ((d->batt_chg[RF_HALF_RIGHT] & 0x3) << 2) |
                       (d->dongle_ok ? 0x10 : 0));
    return RF_DISPLAY_LEN;
}
static inline bool rf_decode_display(const uint8_t *buf, uint16_t len, rf_display_t *o)
{
    if (buf == NULL || o == NULL) return false;
    if (len < RF_DISPLAY_LEN || rf_packet_type(buf, len) != PKT_TYPE_DISPLAY) return false;
    o->couche                   = buf[1];
    o->batt_dv[RF_HALF_LEFT]    = buf[2];
    o->batt_dv[RF_HALF_RIGHT]   = buf[3];
    o->batt_chg[RF_HALF_LEFT]   = buf[4] & 0x3;
    o->batt_chg[RF_HALF_RIGHT]  = (buf[4] >> 2) & 0x3;
    o->dongle_ok                = (buf[4] & 0x10) != 0;
    return true;
}
/* La batterie de l'AUTRE moitié, vue de `moi` (RF_HALF_LEFT/RIGHT). */
static inline void rf_display_autre(const rf_display_t *d, uint8_t moi, uint8_t *dv, uint8_t *chg)
{
    uint8_t autre = (moi == RF_HALF_LEFT) ? RF_HALF_RIGHT : RF_HALF_LEFT;
    *dv = d->batt_dv[autre]; *chg = d->batt_chg[autre];
}
