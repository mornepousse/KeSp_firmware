/* Trames de la sync auto keymap dongle→gauche par ACK payload (fusion, phase 3).
 *
 * Trois trames : BEACON (dongle→gauche dans l'ACK : « j'ai une keymap d'empreinte
 * fp_target en n_chunks »), CHUNK (dongle→gauche dans l'ACK : un morceau de 28 o),
 * REQ (gauche→dongle en uplink : « le prochain chunk que je veux »). Toutes
 * tiennent dans un ACK payload nRF24 (≤ 32 o).
 *
...
static void test_beacon_roundtrip(void)
{
    ...
    /* mauvais type et trame courte rejetés */
...
static void test_chunk_roundtrip(void)
{
    ...
    TEST_ASSERT_EQ(n, 30, "chunk = 30 octets (tient dans un ACK payload)");
    TEST_ASSERT(n <= 32, "≤ 32 octets, la limite nRF24");
...
static void test_req_roundtrip(void)
{
    ...
    TEST_ASSERT(!rf_decode_sync_req(buf, 1, &out), "rejette trop court");
}

/* La géométrie du drip : 40 chunks de 28 o couvrent EXACTEMENT le blob keymap
 * (1120 o), sans chunk partiel — sinon le réassembleur devrait gérer une queue. */
...
{
    TEST_SUITE("Trames de sync keymap (BEACON/CHUNK/REQ)");
