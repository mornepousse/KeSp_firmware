...
**Pinout exact à extraire en début d'implémentation** depuis `~/Documents/PCB-esp/dongle/dongle/dongle.kicad_sch` (les labels existent : `MOSI`, `MISO`, `SCK`, `CSN_1`, `CSN_2`, `CE_1`, `CE_2`, `IRQ_1`, `IRQ_2`, `EN`, `IO0`, `TX`, `RX`) et coder dans `boards/kase_dongle/board.h`. Cette extraction est la 1ère tâche du plan d'implémentation, pas un point de design ouvert.
...
Budget dongle-side (post-on-air) : ~340µs. Plus on-air ~100µs et USB poll 0-1000µs (typique 500µs) = **~940µs end-to-end** côté dongle. Le half ajoute ~200-300µs (scan + packet build + SPI write au NRF) ; budget half-side traité dans le brainstorm halves Phase 2. **End-to-end complet typique : ~1.2-1.4 ms**, pire ~2.5-3 ms (poll USB manqué + 1 retry NRF). Conforme à la cible ≤1 ms typique côté dongle, ≤3 ms p99 end-to-end.
...
**Identification USB** :
- VID/PID dev : `0x303A:0x4001` (Espressif VID + PID arbitraire dans la plage dev) jusqu'à la 1ère release publique. À ce moment, allouer un PID propre via [pid.codes](https://pid.codes/) sous VID `0x1209` (gratuit pour projets open-source).
...
| `key_stats`, `key_stats_tot`, `bigram_stats`, `bigram_total` | KaSe_firmware | Compteurs sur 70 positions (vs 33-35 actuellement). Audit nécessaire des sizes d'array statiques dans `key_stats.c` ; doivent dépendre de `MAX_MATRIX_KEYS` (déjà le cas pour la plupart, à vérifier). Bigram NVS layout reste compatible (clés inchangées, juste le contenu plus grand). |
...
```c
#define COLS_PER_HALF  7   // 5 rows × 7 cols par half
#define POS_PER_HALF   35  // 5 × 7

static inline uint8_t to_global_pos(input_source_t src, uint8_t row, uint8_t col) {
    uint8_t local = row * COLS_PER_HALF + col;
    switch (src) {
        case INPUT_SRC_LOCAL_MATRIX: return local;                    // V1/V2 : ≤35 keys (KaSe non-split)
        case INPUT_SRC_RF_LEFT:      return local;                    // dongle : 0..34
        case INPUT_SRC_RF_RIGHT:     return POS_PER_HALF + local;     // dongle : 35..69
    }
    return 0xFF;  // unreachable, silence compiler
}
```

`COLS_PER_HALF` et `POS_PER_HALF` définis dans `boards/kase_dongle/board.h`. Pour V1/V2/V2D, défini avec les valeurs locales habituelles dans leur `board.h` respectif (pas de breaking change).
...
2. `key_definitions.h` / `keymap.c` : `MAX_MATRIX_KEYS` défini dans `boards/<variant>/board.h` (déjà le cas en V1/V2/V2D : 33). `boards/kase_dongle/board.h` définit `MAX_MATRIX_KEYS=70`. Toutes les structures qui dimensionnent par cette macro (keymaps, key_stats arrays, etc.) suivent automatiquement.
...
- `rf_rx_task.c` : la task qui multiplexe les 2 NRFs, parse les packets, appelle `process_key_event` (PKT_KEY), `trackpad_dispatch` (PKT_TRACKPAD, dans `hid_report.c` ou nouveau `trackpad.c`) ou `heartbeat_reconcile` (PKT_HEARTBEAT).
...
| OTA chunk loss / CRC fail | Re-send 3× max, sinon abort. Half a son propre rollback (assume halves Phase 2 implémentent factory/ota_0 + safe-boot) |
