...
void app_main(void) {
  ...
  xTaskCreate(vTaskTrackBall, "TrackBall", 4096, &ucParameterToPass,
              tskIDLE_PRIORITY, &xHandleTrackBall);
  example_ledc_init();
  xTaskCreate(vTaskLED_Animation, "LED_Animation", 4096, &ucParameterToPass,
              tskIDLE_PRIORITY, &xHandleLEDanim);
  ...
  while (1) {
    ...
    if (tud_mounted()) {
      if (!send_hid_data) {
        // write_txt("Connected", 0, 30);
      }
      send_hid_data = true;
      if (send_hid_data) {
        // show_picture();
      }
      // write_img_nerve_logo();
    }
    if (tud_suspended()) {
      if (send_hid_data) {
        // write_txt("Disconnected", 0, 30);
      }
      send_hid_data = false;
      // DispRGBGray();
    }
