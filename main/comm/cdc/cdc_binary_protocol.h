...
typedef enum {
    ...
    KS_CMD_RF_STATUS        = 0xB3,  /* dongle: link state + signal + packet counters */
    KS_CMD_RF_PAIR_LIST     = 0xB4,  /* dongle: list currently paired half MACs */
    ...
    /* Security key provisioning (dongle) — write-only, no secret read-back (0xC0-0xC3) */
    KS_CMD_SEC_SET_SLOT     = 0xC0,
    KS_CMD_SEC_CLEAR_SLOT   = 0xC1,
    KS_CMD_SEC_LIST         = 0xC2,
    KS_CMD_SEC_SELFTEST     = 0xC3,  /* HMAC-SHA1 KAT (RFC 2202 vec 1) -> 20-byte resp */
