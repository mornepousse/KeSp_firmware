...
- **conchodytes**: mouse (PMW3389), dongle slot 2 — **out of the tree since
  ...
  pinned submodule + CI on `build-board.yml`). **Decided 2026-09-21: the mouse
  gets a dedicated firmware (Rust `esp-hal`, bare metal), not a KeSp role** —
  what it shares with KeSp is a *contract*, not code:
  `docs/DONGLE_MOUSE_CONTRACT.md` + `docs/contracts/mouse_slot_vectors.json`
  (generated by `scripts/gen_mouse_slot_vectors.sh`, pinned by
  `test_mouse_slot_vectors`). The `MOUSE` role code stays here as the working
  reference until the new firmware pairs and clicks on the dongle, then it is
  deleted. The contract also records two inherited facts: derived channels
  80..119 sit above the ISM band (compliance, firmware-wide decision pending),
  and a button held without motion is released by the dongle after 2.5 s
  (the rewrite must keep-alive every `RF_STATUS_PERIOD_MS`)
